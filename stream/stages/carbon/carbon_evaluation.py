...
def calculate_carbon(
    ...
    lifeop_co2 = lifespan*365*24*60*60/taskspan * op_co2
    area_list = {
        "logic": scme.area_total-scme.mem_area, "mem": scme.mem_area
    }
    combinations = list(it.product([scme.carbon_param.technology_node], repeat = len(area_list)))
    area = np.array(list(area_list.values()))   
    design_carbon = np.zeros((len(combinations), len(area_list)+1))
    op_carbon = np.zeros((len(combinations), len(area_list)+1)) 
    ...
    for n, comb in enumerate(combinations): 
        ...
        new_area = area_scaling(area, comb)
        ...
        package_c, router_c, design_carbon[n,-1], router_a = package_costs(scme, area_list, comb, new_area, True, 700)
        carbon[n, -1] = package_c * 1 + router_c
...
def package_costs(scme: "StreamCostModelEvaluation", area_list, technology_node, new_area, return_router_area, carbon_per_kwh):
    package_carbon = 0 
    router_carbon = 0
    router_design = 0
    router_area = 0
